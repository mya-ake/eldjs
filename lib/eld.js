...
EldObject.prototype = {
  ...
  animate: function(params, duration, timing, delay, callback, frame) {
    var t = this;
    var origin = {
      duration: [],
      timing: [],
    ...
    var stepFunc = typeof frame === "function" 
    ? function() {
      frame();
    ...
    function wh(params) {
      ...
      if (params["width"] !== undefined) {
        for (i = t.length - 1; i >= 0; i--) {
          t.elems[i].style.width = t.elems[i].style.width || t.elems[i].clientWidth + "px";
        }
      }
      if (params["height"] !== undefined) {
        for (i = t.length - 1; i >= 0; i--) {
          t.elems[i].style.height = t.elems[i].style.height || t.elems[i].clientHeight + "px";
        }
      }
    }
    ...
    function set(params) {
      Object.keys(params).forEach(function(name) {
        for (var i = t.length - 1; i >= 0; i--) {
          t.elems[i].style[name] = params[name];
        }
      });
    }
