...
Eld.prototype = {
  ...
  selectId: function(id) {
    return new EldObject(document.getElementById(id));
  },
  ...
  selectName: function(name, targetElem) {
    ...
    return new EldObject(targetElem.getElementsByName(name));
  },
  ...
  selectTag: function(tagName, targetElem) {
    ...
    return new EldObject(targetElem.getElementsByTagName(tagName));
  },
  ...
  selectClass: function(className, targetElem) {
    ...
    return new EldObject(targetElem.getElementsByClassName(className));
  },
  ...
  select: function(query, targetElem) {
    ...
    return new EldObject(targetElem.querySelectorAll(query));
  },
  ...
  scrollTop: function(duration, adjust, easing) {
    scrollAnimation(0 + (adjust || 0), duration, easing);
    return this;
  },
};
...
function EldObject(elems) {
  ...
  this.elems = elems;
  this.length = length === null ? elems.length : length;
  this.eld = true;
...
EldObject.prototype = {
  ...
  animate: function(args) {
    ...
    if (t.animateState === null) {
      t.animateState = {t1: null, t2: null, s: false};
    } else {
      clearTimeout(t.animateState.t1);
    }
    t.animateState.s = true;
    t.animateState.t1 = setTimeout(function() {
      t.animateState.s = false;
      stepFunc();
      set(param);
      clearTimeout(t.animateState.t2);
      t.animateState.t2 = setTimeout(function() {
        setOrigin();
        t.animateState.s = true;
        if (typeof args.callback === "function") {
          args.callback();
  ...
  remove: function() {
    if (this.length === 1) {
      removeElem(this.elems[0]);
      return this;
    }
    for (var i = this.length - 1; i >= 0; i--) {
      removeElem(this.elems[i]);
    }    
    return this;
  ...
  screen: function(func, overall) {
    var t = this;
    window.addEventListener("scroll", eventScrollShow, false);
    return t;

    function eventScrollShow(e) {
      ...
      if (overall === true) {
        for (; i < t.length; i++) {
          offsetTop = t.elems[i].offsetTop;
          if (offsetTop >= top && (offsetTop + t.elems[i].clientHeight) <= bottom) {
            flg = func(e, t.elems[i]);
          }
        }
      } else {
        for (; i < t.length; i++) {
          offsetTop = t.elems[i].offsetTop;
          if ((offsetTop + t.elems[i].clientHeight) >= top && offsetTop <= bottom) {
            flg = func(e, t.elems[i]);
          }
        }
      }
      if (flg === false) {
        window.removeEventListener("scroll", eventScrollShow, false);
      }
    }
  },
  
  screenOut: function(func, overall) {
    var t = this;
    window.addEventListener("scroll", eventScrollHide, false);
    return t;

    function eventScrollHide(e) {
      var top = document.documentElement.scrollTop || document.body.scrollTop;
      var bottom = top + window.innerHeight;
      var flg;
      var offsetTop;
      ...
      if (flg === false) {
        window.removeEventListener("scroll", eventScrollHide, false);
      }
    }
  },
  
  scrollTop: function(duration, adjust, easing) {
    scrollAnimation(this.elems[0].offsetTop + (adjust || 0), duration, easing);
    return this;
  },
};
...
function scrollAnimation(to, time, easing) {
  var INTERVAL = 16;
  ...
  var easingFunc = easeOut;
  if (easing === "linear") {
    easingFunc = linear;
    d = d < 0 ? change : change * -1;
  }
  var siid = setInterval(function() {
    now = easingFunc(counter, s, d, count);
